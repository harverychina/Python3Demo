...
class OptSqlite(object):
    ...
    def get_one_phone(self, username):
        ...
        获取一个联系人的电话
        """
        self.get_user_phone_sql = f"""SELECT phone FROM address_book WHERE name ="{username}" """
    ...
    def set_one_phone(self, name, phone):
        ...
        if self.get_one_phone(name):
            self.set_user_phone_sql = '''UPDATE address_book SET phone = ? WHERE name = ?'''
            self.v = (int(phone), str(name))
        else:
            self.set_user_phone_sql = '''INSERT INTO address_book VALUES(?, ?, ?)'''
    ...
    def delete_one_content(self, name):
        ...
        self.delete_user_sql = f'''DELETE FROM address_book WHERE name = "{name}" '''
...
# 查询所有人的电话
contens = my_query.get_all_contents()
print(contens)
